...
const api_key = process.env.npm_package_apiKey_secret || '';
...
class ImageProvider extends React.Component {
  constructor(props) {
    ...
    this.handleScroll = (e) => {
      if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight) {
            if(parseInt(this.state.original.total/20)+(this.state.original.total%20 !==0 ? 1 : 0) > this.pageCount){
              this.pageNext();
            }
      }
    }
  ...
  updateData(term) {
		if(api_key.length == 0){
			let toastid = toast.info("No API-KEY", {
	      position: toast.POSITION.TOP_CENTER,
	      autoClose:false
	    });
		}
  ...
  render() {
    return (
      <ImageContext.Provider
        value={{ images: this.state.images, updateData: this.updateImageData, search: this.search }}
      >
        {this.props.children}
      </ImageContext.Provider>
    );
